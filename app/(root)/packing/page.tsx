...
const PackingPage: React.FC = () => {
  ...
  const [categories, setCategories] = useState<Category[]>(() => {
    if (typeof window !== "undefined") {
      const storedData = localStorage.getItem("packingList");
  ...
  const handleAddItem = useCallback((): void => {
    if (!itemName.trim() || !quantity.trim() || !selectedCategory) return;
    ...
    setCategories((prevCategories: Category[]) => {
      ...
      if (!updatedCategories.some((cat) => cat.name === selectedCategory)) {
        updatedCategories.push({
          ...
          items: [{ name: itemName, quantity, checked: false }],
        });
      }
  ...
  }, [itemName, quantity, selectedCategory]);
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {[...categories]
        .sort((a: Category, b: Category) => {
          const aChecked = a.items.every((item) => item.checked);
          const bChecked = b.items.every((item) => item.checked);
